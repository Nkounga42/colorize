  ...
  const LayersPanel = ({
    ...
    // FONCTION MODIFIÉE : handleRename avec sauvegarde dans l'objet Fabric.js
    const handleRename = (layerId, name) => {
      const updatedLayers = layers.map((layer) => {
        if (layer.id === layerId) {
          // Stocker le nom personnalisé dans l'objet Fabric.js
          layer.object.customName = name.trim();
  ...
  // Vérifier d'abord dans les sélections multiples
  if (selectedLayers.length > 0) {
    return selectedLayers.some(selected => selected.id === layer.id);
  }
  // Sinon vérifier la sélection simple
  return selectedLayer === layer.object;
};
            ...
            <div className="flex items-center justify-between">
              ...
              {renamingLayerId !== layer.id  && <div className="flex items-center space-x-1">
                <div className="flex flex-col">
                  ...
                  className=" not-only-of-type: text-gray-400 hover:text-gray-600"
                  title="Monter"
                >
                  <ChevronUp size={14} />
                </button>
...
useEffect(() => {
  ...
  // Permettre la sélection multiple avec Ctrl+Clic sur le canvas
  canvas.on('mouse:down', (options) => {
    if (options.e.ctrlKey || options.e.metaKey) {
      ...
      if (clickedObject) {
        ...
        if (activeObjects.includes(clickedObject)) {
          ...
          // Ajouter l'objet à la sélection
          const newSelection = [...activeObjects, clickedObject];
          if (newSelection.length > 1) {
            const activeSelection = new fabric.ActiveSelection(newSelection, {
              canvas: canvas,
            });
...
}, [selectedLayers, deleteSelectedLayers]);
